...
export default function DetallePelicula() {
    ...
    function obtenerURLEmbebidaYotube(url: string): string | undefined {
            ...
            <div className="container my-4">
                <div className="mb-3">
                    ...
                    {pelicula.generos && pelicula.generos.length > 0 && (
                        <div className="mb-2">
                            {pelicula.generos.map(genero => <span key={genero.id} className="badge bg-primary me-2">
                                {genero.nombre}
                            </span>)}
                        </div>
                    )}

                    <p className="text-muted">Estreno: {fechaFormateada} | Puntuacion promedio: {pelicula.promedioVoto} | Mi rating <Rating maximoValor={5} valorSeleccionado={pelicula.votoUsuario} onChange={voto => manejarVoto(voto)} /></p>
                </div>

                <div className="d-flex">
                    <span className="d-inline-block me-2">
                        <img src={pelicula.poster} style={{ width: '225px', height: '315px' }} alt={pelicula.titulo} />
                    </span>
                    <div>
                        {pelicula.trailer && pelicula.trailer.includes('youtu.be/') ? (
                            <iframe
                                width="565"
                                height="315"
                                src={`https://www.youtube.com/embed/${pelicula.trailer.split('youtu.be/')[1]?.split('?')[0]}`}
                                title="Trailer"
                                allowFullScreen
                            ></iframe>
                        ) : (
                            <p className="text-muted">Trailer no disponible</p>
                        )}
                    </div>
                </div>
